 ...
 * @since 1.0.0
 */
public class EnumEqValid extends ValidatorHandler<BaseEnum> implements Validator<BaseEnum> {

    private BaseEnum expectValue;
    private Class enumClass;

    public EnumEqValid(Class enumClass, BaseEnum expectValue) {
    ...
    public boolean validate(ValidatorContext context, ValidatorElement element, BaseEnum t) {
        ...
        if (targetEnum == null) {
            ...
            String errorMsg = "期望 %s 枚举等于 %s，但实际值为 %s";
...
/*    public static void main(String[] args) {
        Msg result = ValidUtils.checkAll()
                .on(TodoStatus.NOT_STARTED, new EnumEqValid(TodoStatus.class, TodoStatus.COMPLETED)).alias("todoStatus", "待办状态")
                .doValidate()
                .result(Msg.toMsg());
        System.out.println(result);
        Msg result2 = ValidUtils.checkAll()
                .on(TodoStatus.COMPLETED, new EnumEqValid(TodoStatus.class, TodoStatus.COMPLETED)).alias("todoStatus", "待办状态")
                .doValidate()
                .result(Msg.toMsg());
