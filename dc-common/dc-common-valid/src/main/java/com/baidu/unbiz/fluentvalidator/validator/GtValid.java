 ...
 * @since 1.0.0
 */
public class GtValid extends ValidatorHandler<Number> {

    private Number expectValue;

    public GtValid(Number expectValue) {
    ...
    public boolean validate(ValidatorContext context, ValidatorElement element, Number t) {
        ...
        if (t.doubleValue() < this.expectValue.doubleValue()) {
            String errorMsg = "期望 %s 大于 %s，但实际值为 %s。";
            context.addError(
    ...
    public static void main(String[] args) {
        Msg result = ValidUtils.checkAll().key("privateId")
                .on(4, new GtValid(3)).alias("privateId", "待办外键")
                .doValidate()
                .result(Msg.toMsg());
