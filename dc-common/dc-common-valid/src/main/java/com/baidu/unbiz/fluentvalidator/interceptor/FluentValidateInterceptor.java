...
public class FluentValidateInterceptor implements MethodInterceptor, InitializingBean, ApplicationContextAware {
     ...
     * {@link ValidUtils#putAttribute2Context(String, Object)}
     * <p/>
     * 这里是做扩展使用的
     */
                ...
                for (int i = 0; i < paramAnnotations.length; i++) {
                    ...
                    for (int j = 0; j < paramAnnotation.length; j++) {
                        if (paramAnnotation[j].annotationType() == FluentValid.class) {
                            ...
                            ValidChain addOnValidatorChain =
                            ...
                            ValidUtils fluentValidator = ValidUtils.checkAll(groups)
    ...
    /**
     * 将需要额外验证的验证器封装为{@link ValidChain}
     * @param fluentValid 参数上的注解装饰
     * @return 验证器链
     */
    private ValidChain getAddOnValidatorChain(FluentValid fluentValid) {
        ValidChain chain = new ValidChain();
