 ...
 * <pre>
 * Result ret = ValidUtils.checkAll().configure(new SimpleRegistry())
 *         .on(car)
 *         .doValidate()
 *         .result(toSimple());
 * </pre>
...
public class QuickValidator {
    ...
    /**
     * Execute validation by using a new ValidUtils instance with a shared context.
    ...
    public static ComplexResult doAndGetComplexResult(Decorator decorator, ValidatorContext context) {
        return validate(decorator, ValidUtils.checkAll(), context, ResultCollectors.toComplex());
    }

    /**
     * Execute validation by using a new ValidUtils instance without a shared context.
     * The result type is {@link ComplexResult}
     * @param decorator Same as decorator design pattern, provided to add more functions to the fluentValidator
     * @param context Validation context which can be shared
     * @return ComplexResult
    ...
    public static ComplexResult doAndGetComplexResult(Decorator decorator) {
        return validate(decorator, ValidUtils.checkAll(), null, ResultCollectors.toComplex());
    }
    ...
    public static ComplexResult2 doAndGetComplexResult2(Decorator decorator, ValidatorContext context) {
        return validate(decorator, ValidUtils.checkAll(), context, ResultCollectors.toComplex2());
    }

    /**
     * Execute validation by using a new ValidUtils instance and without a shared context.
     * The result type is {@link ComplexResult2}
     * @param decorator Same as decorator design pattern, provided to add more functions to the fluentValidator
     * @return ComplexResult2
     */
    public static ComplexResult2 doAndGetComplexResult2(Decorator decorator) {
        return validate(decorator, ValidUtils.checkAll(), null, ResultCollectors.toComplex2());
    }

    /**
     * Use the <code>decorator</code> to add or attach more functions the given <code>fluentValidator</code> instance.
     * <p>
     * The context can be shared and set up in the new ValidUtils instance.
     * <p>
     ...
     * @return Result
     */
    public static <T extends GenericResult<ValidError>> T validate(Decorator decorator, ValidUtils fluentValidator, ValidatorContext context, ResultCollector<T> resultCollector) {
        ValidUtils fv = decorator.decorate(fluentValidator);
        if (context != null) {
